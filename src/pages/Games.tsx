...
import { Brain, Trophy, Target, Zap, Star, Gamepad2, ArrowRight, Lock } from "lucide-react";
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          return <Card key={game.id} className={`rounded-3xl border-2 shadow-float transition-all duration-300 hover:shadow-bubble hover:scale-105 ${game.available ? 'cursor-pointer' : 'opacity-60'}`}>
                <CardHeader className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge variant="outline" className={`rounded-full bg-white border-2 ${game.color}`} style={{
                    borderColor: game.id === 'quiz' ? 'hsl(var(--primary))' : game.id === 'memory' ? 'hsl(var(--secondary))' : game.id === 'challenge' ? 'hsl(var(--warning))' : 'hsl(var(--success))'
                  }}>
                        {game.difficulty}
                      </Badge>
                      {!game.available && <Badge variant="outline" className="rounded-full">
